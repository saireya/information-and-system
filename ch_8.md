...
代わりに、部分集合を表す記号「$\subset$」を使い、この命題は「$\text{ネコ} \subset \text{動物}$」と表せる。
...
また、集合$X$の *ある* 要素$x$について命題$P_x$が成り立つ($P_x$が成り立つような$x$が *存在する* )ことを、特称記号と呼ばれる記号「$\exists$」を使って「$\exists x \in X, P_x$」と表す。
...
- $P$を *確率的* に示す
		...
		例えば、高い偶発性をもつなど極めて特殊な条件下でしか反例が生じない場合は、そのような場合を検討する必要性や意義が小さい。
