...
$P, Q$が命題のとき、「$P$ならば$Q$である」という形の命題を、「ならば」の意味を表す記号「$⇒$」を用いて「$P ⇒ Q$」と表す。
...
代わりに、部分集合を表す記号「$\subset$」を用い、この命題は「$\text{ネコ} \subset \text{動物}$」と表せる。
...
集合$X$の *すべての* ( *任意の* )要素$x$について命題$P_x$が成り立つことを、全称記号と呼ばれる記号「$\forall$」を用いて「$\forall x \in X, P_x$」と表す。
また、集合$X$の *ある* 要素$x$について命題$P_x$が成り立つ($P_x$が成り立つような$x$が *存在する* )ことを、特称記号と呼ばれる記号「$\exists$」を用いて「$\exists x \in X, P_x$」と表す。これらを、前のコラムで用いた「$:=$」「$\wedge$」「$\vee$」を用いて形式的に定義すると、次のようになる。
...
>
> この定義を用いると、「すべての$x$について$P_x$が成り立つ」の否定が「ある$x$について$P_x$が成り立たない」になることや、「ある$x$について$P_x$が成り立つ」の否定が「すべての$x$について$P_x$が成り立たない」になることは、次の式変形で示せる。変形の途中ではde Morganの公式($\overline{A \wedge B} = \bar{A} \vee \bar{B}, \overline{A \vee B} = \bar{A} \wedge \bar{B}$)を用いている。
...
3種の推論法は、推論で得られた結論がどの程度正しいかという *厳密性* と、推論で得られた結論がどの程度元の命題にないことを述べているかという *拡張性* の高さが、それぞれ異なっている。
...
実際に命題$P$の正しさを示す場面では、次の方法も用いられる。
...
- $P$を *実験* や *調査* 、 *シミュレーション* 、 *統計* で示す
		...
		物理学や生物学、心理学や社会学では、演繹では正しさを示せない場合が多いため、この方法を用いることが一般的である。
