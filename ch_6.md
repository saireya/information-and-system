...
$e_{kl}$が存在すれば$\epsilon_{kl} = 1$、存在しなければ$\epsilon_{kl} = 0$と定義すると、$\epsilon_{ij} (i = 1, \cdots, n; j = 1, \cdots, n)$を並べた行列$\Epsilon = (\epsilon_{ij})$を定義できる。
...
このように、 *システム全体の能力が各要素の能力の和と等しくならない* 性質を指して、システムは **創発的** (**emergence**)であるという\[[broad]\]\[[maraterre]\]。
